...
function SidebarContent({ navigation, pathname, user, onLogout }: SidebarContentProps) {
  return (
          ...
          return (
            <Link
              ...
              className={clsx(
                ...
                "group flex items-center px-4 py-3 text-base font-medium border-l-4 rounded-r-md transition-colors duration-200",
              )}
            >
              <item.icon className={clsx(isActive ? "text-green-500" : "text-gray-400 group-hover:text-gray-500", "mr-4 h-6 w-6")} />
              {item.name}
            </Link>
          );
        })}
      ...
      <div className="flex-shrink-0 flex border-t border-gray-200 p-4">
        <div className="flex-shrink-0 w-full group block">
          <div className="flex items-center">
            ...
            <button onClick={onLogout} className="ml-3 text-gray-400 hover:text-gray-600 transition-colors duration-200" title="Logout">
